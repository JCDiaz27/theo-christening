...
// ─────────────────────────────────────────
// SCROLL BEHAVIOUR — fade on scroll, no snap
// ─────────────────────────────────────────
(function () {
  ...
  const sections    = [heroSection, rsvpSection];

  if (!heroSection || !rsvpSection) return;
  ...
  dots.forEach(dot =>
    dot.addEventListener('click', () =>
      sections[Number(dot.dataset.index)]?.scrollIntoView({ behavior: 'smooth' })
    )
  );
  ...
  const dotObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) refreshDots(sections.indexOf(entry.target));
    });
  }, { threshold: 0.5 });
  sections.forEach(s => dotObserver.observe(s));

  // ── Fade-in RSVP card when it scrolls into view ──────────────────────────
  if (rsvpCard) {
    rsvpCard.classList.remove('fade-in', 'delay-1'); // prevent instant play on load
    rsvpCard.classList.add('scroll-reveal');
    ...
    revealObserver.observe(rsvpCard);
  }
